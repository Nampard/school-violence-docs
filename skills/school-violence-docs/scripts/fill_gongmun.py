...
종류 6종과 분기:
  ...
  자체해결내부   #4 자체해결 결과 보고 [내부결재]      [자체해결]     내부결재
  자체해결청     #5 자체해결 결과 보고 [교육지원청]    [자체해결]     교육지원청 발송
  종결내부      #4 종결 결과 보고 [내부결재]         [종결]        내부결재    ※문구만 '종결'로
  종결청        #5 종결 결과 보고 [교육지원청]       [종결]        교육지원청 발송 ※문구만 '종결'로
  심의위        #6 심의위원회 개최 요청              [심의위 이송]  교육지원청 발송

※ 양식 #4·#5는 제목이 '자체해결(종결)'·'자체해결'로 되어 있어 종결 사안에 그대로 쓰면 어색하다.
  종결내부/종결청을 쓰면 같은 쪽을 쓰되 '자체해결(종결)'→'종결', '자체해결 결과 보고'→'종결 결과 보고'로
  바꿔 준다(붙임의 '학교장 자체해결(종결) 동의서'도 '학교장 종결 동의서'가 되어 종결 동의서 양식명과 맞는다).
...
# 종결내부/종결청은 자체해결과 같은 쪽(4·5쪽)을 쓰되, 문구를 '종결'로 바꿔 쓴다.
KIND_PAGE = {"접수": 0, "개최": 1, "결과": 2,
             "자체해결내부": 3, "자체해결청": 4,
             "종결내부": 3, "종결청": 4,
             "심의위": 5}
# 종결 분기 문구 치환(더 구체적인 것부터). 4쪽 제목·본문·붙임의 '자체해결(종결)'과
# 5쪽 제목·본문·붙임의 '자체해결 결과 보고'를 종결용으로 바꾼다.
JONGGYEOL_SUB = [("자체해결(종결)", "종결"), ("자체해결 결과 보고", "종결 결과 보고")]
...
def main(template, data_path, out_path):
    ...
    for p in keep:
        ...
        if kind in ("종결내부", "종결청"):                       # 자체해결 문구 → 종결 문구
            for a, b in JONGGYEOL_SUB:
                new = new.replace(a, b)
